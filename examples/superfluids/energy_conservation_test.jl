...
function s_init(x::RealVector)::Float64
    return s_max*sin(pi*x[1]/L)*sin(pi*x[2]/L)
end

function s_exact(x::RealVector, t::Float64)::Float64
    return s_max*sin(pi*x[1]/L)*sin(pi*x[2]/L)*cos(sqrt(2)*pi*u2*t/L)
end
...
function make_system(gp::GlobalParameters)::ParticleSystem
    ...
    generate_particles!(sys, grid, dom, x -> Particle(x=x, s=s_init(x)))
...
function find_Drho_and_Ds!(p::Particle, q::Particle, r::Float64, gp::GlobalParameters)
    ...
    x_pq = p.x - q.x
    gradw_p = ker*p.R*x_pq
    gradw_q = ker*q.R*x_pq
    ...
    p.Ds   += -gp.m*(dot(j_p, gradw_p)/(p.rho^2) + dot(j_q, gradw_q)/(q.rho^2))
end
...
function find_a_and_as!(p::Particle, q::Particle, r::Float64, gp::GlobalParameters)
    ...
    vns_p = v_ns(p)
    vns_q = v_ns(q)
    pressure = -gp.m*(p.P/p.rho^2*gradw_p + q.P/q.rho^2*gradw_q)
    p.a += pressure
    p.a += - gp.m*p.xn*p.xs/p.rho*dot(gradw_p, vns_p)*vns_p
    p.a += - gp.m*q.xn*q.xs/q.rho*dot(gradw_q, vns_q)*vns_q
    p.as += pressure
...
function main(N::Int64)
    ...
    energy_data = open("results/waves_nonlinear/energy_data"*string(N)*".csv", "w")
    e_char = find_energy(sys, gp)[end]
		...
		@printf("t = %.6e\n", t)
                ...
                for p in sys.particles
                    p.s_exact = s_exact(p.x,t)
                end
		save_frame!(out, sys, :v, :vs, :P, :T, :xn, :xs, :s, :type, :s_exact)
	    end
	    apply_with_gp!(sys, accelerate!, gp)
